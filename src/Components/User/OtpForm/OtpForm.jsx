...
function OtpForm() {
  ...
  const Completionist = () => <button style={{width:"60px",fontSize:"0.7rem",marginTop:"5px",backgroundColor:"grey"}} onClick={resendOtp}>Resend</button>;
...
<form onSubmit={handleSubmit}>
      ...
      <p style={{fontSize:"0.8rem"}}>Otp verification code is sent to your gmail</p>
      <Form.Control size="sm" type="number" onChange={handleChange} name="otpnumber" placeholder="ENTER OTP"/>
      {formErrors.otpError&&<div style={{textAlign:"left"}}><p style={{fontSize:"0.8rem",color:"red"}}>{formErrors.otpError}</p></div>}
     
      {invalidOtp&&<div style={{textAlign:"left"}}><p style={{fontSize:"0.8rem",color:"red"}}>{invalidOtp}</p></div>}
